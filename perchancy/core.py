...
class BrowserCore:
    ...
    def init_driver(self) -> None:
        ...
        self.log("Pre-loading domain for cookies...")
        self.page.get("https://perchance.org/")
        self.load_cookies()
    ...
    def execute(self, model: str, prompt: str, is_image: bool = False, num_images: int = 1, stream: bool = False, extra_params: Optional[Dict[str, Any]] = None, param_mappings: Optional[Dict[str, Union[str, List[str]]]] = None, input_selectors: Optional[List[str]] = None, button_selectors: Optional[List[str]] = None, output_selectors: Optional[List[str]] = None, disable_safety_settings: bool = False) -> Any:
        ...
        self.log(f"Opening new tab for generator model: https://perchance.org/{model}")
        ...
        def process_request():
            try:
                self.log("Searching for the generator iframe in new tab...")
                    ...
                    if stream:
                        def stream_generator() -> Generator[str, None, None]:
                            try:
                                ...
                                self.log("Stream ended, closing tab.")
                                tab.close()
        ...
        if stream and not is_image:
            return process_request()
        else:
            try:
                return process_request()
            finally:
                self.log("Request finished, closing tab.")
                tab.close()